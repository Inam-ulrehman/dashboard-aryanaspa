...
const LocalStorageUploadImage = getItemFromLocalStorage('sectionThreeImage')
...
const ContentSectionThree = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      removeItemFromLocalStorage('sectionThreeImage')
  ...
  const cbFunction = async (result) => {
    const name = 'sectionThreeImage'
    const uploadImage = result.data.sectionThree.uploadImage
    if (state._id === 0) {
      setItemInLocalStorage(name, uploadImage)
    }
    setState({ ...state, uploadImage: result.data.sectionThree.uploadImage })
  }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = result?.data?.sectionThree
